(function attachDailyReport(globalScope) {
  ...
  function renderClientTwoProfile(report, clientReport = report) {
    ...
    const renderDashboardDevelopers = () => {
      ...
      target.innerHTML = developers.map(([name, items]) => { const active = items.filter(isActive).length; const overdue = items.filter((task) => deadlineState(task) === 'overdue').length; return `<div class="client-two-dashboard-item"><div><strong>${escapeHtml(name)}</strong><span>${items.length} задач</span></div><small>${active} в роботі · ${overdue} прострочено</small><i class="client-two-meter" tabindex="0" aria-label="${escapeHtml(name)}: ${items.length} задач"><b style="width:${Math.max(6, Math.round((items.length / Math.max(1, tasks.length)) * 100))}%"></b><span class="client-two-dashboard-popover" role="tooltip"><strong>${escapeHtml(name)}</strong><span>${items.length} задач · ${active} в роботі · ${overdue} прострочено</span><small>${items.slice(0, 8).map((task) => escapeHtml(task.title)).join('<br>')}${items.length > 8 ? '<br>… та інші' : ''}</small></span></i></div>`; }).join('') || '<p class="empty">Розробників не вказано.</p>';
    };
    const renderDashboardBlocks = () => {
      ...
      target.innerHTML = blockNames.map((name) => { const items = tasks.filter((task) => task.block === name); const overdue = items.filter((task) => deadlineState(task) === 'overdue'); const review = items.filter((task) => isReview(task) && !overdue.includes(task)); const active = items.filter((task) => isActive(task) && !overdue.includes(task) && !review.includes(task)); const other = items.filter((task) => !overdue.includes(task) && !review.includes(task) && !active.includes(task)); const total = Math.max(1, items.length); const popoverItems = items.slice(0, 7).map((task) => escapeHtml(task.title)).join('<br>'); return `<article class="client-two-block-status-row client-two-dashboard-hover"><div class="client-two-block-status-head"><strong>${escapeHtml(name)}</strong><b>${items.length}</b><span>задач</span></div><i class="client-two-block-status-bar" aria-label="${escapeHtml(name)}: ${items.length} задач"><b class="is-overdue" style="width:${(overdue.length / total) * 100}%"></b><b class="is-review" style="width:${(review.length / total) * 100}%"></b><b class="is-active" style="width:${(active.length / total) * 100}%"></b><b class="is-other" style="width:${(other.length / total) * 100}%"></b><span class="client-two-dashboard-popover" role="tooltip"><strong>${escapeHtml(name)}</strong><span>${items.length} задач</span><small>Прострочено: ${overdue.length}<br>На перевірці: ${review.length}<br>В роботі: ${active.length}<br>Інше: ${other.length}<br><br>${popoverItems}${items.length > 7 ? '<br>… та інші' : ''}</small></span></i><small class="client-two-block-status-note">${overdue.length} прострочено · ${active.length} в роботі · ${review.length} на перевірці</small></article>`; }).join('') || '<p class="empty">Блоки ще не визначені.</p>';
    };
    const renderDashboardServices = () => {
      ...
      const maxServiceTasks = services[0]?.[1].length || 1;
      target.innerHTML = services.map(([key, items]) => { const [block, service] = key.split('|||'); const people = [...new Set(items.flatMap((task) => splitPeople(task.developer)))].filter((person) => person !== 'інформація відсутня'); const active = items.filter(isActive).length; const bugs = items.filter((task) => task.type === 'bug').length; const enhancements = items.filter((task) => task.type === 'enhancement').length; const fresh = items.filter((task) => task.type === 'new').length; const overdue = items.filter((task) => deadlineState(task) === 'overdue').length; return `<article class="client-two-service-card"><div class="client-two-service-card__head"><strong>${escapeHtml(service)}</strong><b>${items.length}</b></div><small>${escapeHtml(block)}</small><div class="client-two-service-card__scale"><i style="width:${Math.max(8, Math.round((items.length / maxServiceTasks) * 100))}%"></i></div><div class="client-two-service-card__legend"><span><i class="is-active"></i>${active} в роботі</span><span><i class="is-bug"></i>${bugs} багів</span><span><i class="is-enhancement"></i>${enhancements} допрац.</span><span><i class="is-new"></i>${fresh} нових</span><span><i class="is-overdue"></i>${overdue} простр.</span></div><span class="client-two-service-card__people">${escapeHtml(people.join(', ') || 'Виконавця не вказано')}</span></article>`; }).join('') || '<p class="empty">Сервіси ще не визначені.</p>';
    };
    renderDashboardBars(); renderDashboardBlocks();
    renderAll();
